...
%>
<form id="pagerForm" method="post" action="<%=basePath%>currencyPurchase/currencyPurchaseList">
...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            <%--<li><a class="add" href="<%=basePath%>currencyPurchase/update" target="dialog" mask="true"><span>添加</span></a></li>--%>
            <!-- 用于数组传送数据 -->
            <!--
            ...
            <%--<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" postType="string"--%>
                   <%--href="<%=basePath%>currencyPurchase/delete?ids=${id}" class="delete" target="ajaxTodo" title="确定要删除吗？"--%>
                   <%--warn="请选择一个用户"><span>批量删除</span></a></li>--%>
            <%--<li><a class="delete" href="<%=basePath%>currencyPurchase/delete?ids={id}" target="ajaxTodo" title="确定要删除吗？"--%>
                   <%--warn="请选择一个用户"><span>删除</span></a></li>--%>
            <li><a class="edit" href="<%=basePath%>currencyPurchase/update?id={id}" target="dialog" mask="true"
        ...
        <table class="table" width="100%" targetType="navTab" asc="asc" desc="desc">
            ...
            <c:forEach items="${page.list}" var="pojo">
                <tr target="id" rel="${pojo.id}">
                    ...
                    <td>${pojo.id}</td>
                    <td>${pojo.customerId}</td>
                    <td>${pojo.currencyId}</td>
                    <td>${pojo.num}</td>
                    <td>${pojo.purchaseDate}</td>
                    <td>
                        <a title="确认删除吗?" target="ajaxTodo" href="<%=basePath%>currencyPurchase/delete?ids=${pojo.id}"
                           class="btnDel">删除</a>
                        <%--<a title="编辑" target="dialog" mask="true" href="<%=basePath%>currencyPurchase/update?id=${pojo.id}"--%>
                           <%--class="btnEdit">编辑</a>--%>
                    </td>
                </tr>
            </c:forEach>
