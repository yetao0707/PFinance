	...
	$.extend($.fn, {
		comboxSelect: function(options){
			...
			return this.each(function(){
				...
				if(box.attr("disabled")){// 增加combox对disabled属性的支持
					return;
				}
		...
		combox:function(){
			...
			return this.each(function(i){
				...
				var disabled = $this.attr("disabled");// 增加combox对disabled属性的支持
				
				var cid = $this.attr("id") || Math.round(Math.random()*10000000);
				var select = '<div class="combox"><div id="combox_'+ cid +'" class="select"' + (ref?' ref="' + ref + '"' : '') +  (disabled?' disabled="' + disabled + '"' : '') + '>';// 增加combox对disabled属性的支持
				...
				$("option", $this).each(function(){
					var option = $(this);
					options +="<li><a class=\""+ (value==option[0].value?"selected":"") +"\" href=\"javascript:void(0)\" value=\"" + option[0].value + "\">" + option[0].text + "</a></li>";// 修改href地址，解决firefox等浏览器处理的bug
				});
				options +="</ul>";
				...
				if (ref && refUrl&&!disabled) {// 增加combox对disabled属性的支持
