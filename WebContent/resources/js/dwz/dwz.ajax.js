...
function _iframeResponse(iframe, callback){
	...
	$iframe.bind("load.jcarousel", function(event){
...
function dialogAjaxDone(json){
	...
	if (json.statusCode == DWZ.statusCode.ok){
		...
		if("closeAll" == json.callbackType){// 关闭所有dialog
...
function dwzPageBreak(options){
	...
	if (op.rel) {
		...
		if(null == form){// 支持navtab页面中存在tab页时，不同的tab页对应同一个查询查询表单时tab页分页
 ...
 * @param {Object} form
 */
function submit4NavTab(form,rel,title){
	var $form = $(form);
	if (!$form.valid()) {
		return false;
	}
	if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
	navTab.openTab(rel, $form.attr("action"),{title:title,data: $form.serializeArray(),type:"POST"});
	return false;
}