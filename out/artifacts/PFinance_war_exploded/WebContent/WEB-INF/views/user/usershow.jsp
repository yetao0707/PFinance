...
<script type="text/javascript">
	...
	function ajaxFileUpload() {
		...
		var id = $("#userid").val(); //flag为id、name属性名
		
		$.ajaxFileUpload({
			url : 'userUpload?id=' + id,
			type : 'post',
...
<body scroll="no">
	...
	<div align="center" style="width: 100%;height: 100%;">
		<input id="issubmit" value="${vo.isSubmit }" hidden="true" />
		<tab:tabContainer id="task">
				...
				<form id="userform" enctype="multipart/form-data" onsubmit="false">
					...
					<tr>
						...
						<td width="30"><input type="text"
							style=" height:auto; width:auto" name="name" value="${vo.name }" /></td>
						<td colspan="2">性别</td>
						...
						<td colspan="2"><input type="text"
							...
							value="${vo.nation }" /></td>
						<td colspan="12" rowspan="6"><img src="${vo.pictureUrl }" width="50%"/><br />
						<div>
								<input type="file" id="file" name="file"><br /> <input
									type="button" id="btnUpload" value="上传头像" />
							</div></td>
					</tr>
					<tr>
						<td>学历</td>
					...
					<input id="userbutton" type="submit" value="保存" />
				</form>
							...
							<tbody>
								<c:forEach items="${vo.resumes }" var="resume"
									...
									<tr>
										...
										<td><input name="resumes[${status.getIndex()}].worktime"
								...
								<c:if test="${vo.resumes.size()==0}">
									<c:forEach begin="0" end="3" varStatus="i">
										<tr>
											<td><input name="resumes[${i.getIndex()}].id" value="-1"
												hidden="true"></td>
											<td><input name="resumes[${i.getIndex()}].worktime"
												value="" /></td>
											<td><input name="resumes[${i.getIndex()}].description" /></td>
						...
						<tbody align="left">
							<c:forEach items="${vo.sciences }" var="science"
								...
								<tr>
									...
									<td><input name="sciences[${status.getIndex()}].awardName"
										value="${science.awardName}"></td>
									<td><input name="sciences[${status.getIndex()}].awardDep"
										value="${science.awardDep}"></td>
									<td><input name="sciences[${status.getIndex()}].awardDate"
										value="${science.awardDate}"></td>
									<td><input name="sciences[${status.getIndex()}].awardNum"
							...
							<c:if test="${vo.sciences.size()==0}">
								<c:forEach begin="0" end="3" varStatus="i">
									<tr>
										<td><input name="sciences[${i.getIndex()}].id" value="-1"
											hidden="true"></td>
									...
									<c:if test="${vo.funds.size()==0}">
										<c:forEach begin="0" end="3" varStatus="i">
											<tr>
												...
												<td><input name="funds[${i.getIndex()}].projectPostion"></td>
											</tr>
										</c:forEach>
									</c:if>
								</tbody>
										...
										<c:forEach begin="0" end="3" varStatus="i">
											<tr>
												<td><input name="projects[${i.getIndex()}].id"
													value="-1" hidden="true"></td>
												<td><input name="projects[${i.getIndex()}].projectName"
													value=""></td>
