function initEnv() {
	...
	setTimeout(function(){
		...
		//加载用户首页
		navTab.reload('getUsersHomePage', {}, 'main');
	}, 10);
...
function initUI(_box){
	...
	$("div.buttonPurple", $p).hoverClass("buttonPurpleHover");
	$("div.buttonGreen", $p).hoverClass("buttonGreenHover");
	$("div.buttonSilver", $p).hoverClass("buttonSilverHover");
	...
	if ($.fn.datepicker){
		$('input.date', $p).each(function(){
			...
			if ($this.attr("relMinDate")) {
				opts.relMinDate = $this.attr("relMinDate");
			}
	...
	$("a[target=dialog]", $p).each(function(){
		$(this).click(function(event){
			...
			// 支持dialog手动设置显示位置
			var iLeft = $this.attr("left");
			var iTop = $this.attr("top");
			if (iLeft) options.left = iLeft;
			if (iTop) options.top = iTop;
			// end
	...
	$("a[target=ajaxEncodeUri]", $p).each(function(){
		$(this).click(function(event){
			var $this = $(this);
			var rel = $this.attr("rel");
			if (rel) {
				var $rel = $("#"+rel);
				var url = encodeURI(encodeURI($this.attr("href")));
	...
	if ($.fn.commonDetail) $("table.commonDetail", $p).commonDetail();
